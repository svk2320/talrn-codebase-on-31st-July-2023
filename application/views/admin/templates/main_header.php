...
.ms-1 {
  ...
  $user_type = ($_SESSION['registered_as'] == 1) ? "organisation" : "individual";
    ...
    <?php if(isset($announcement[$user_type])): ?>
    <div class="alert text-white alert-dismissible fade show rounded-0 mb-1 text-lg-center" role="alert"
      style="z-index: 1038;background-color: #5473FF;">
      <div class="container">
      ...
      <!-- /.container -->
      <button type="button" class="btn-close" data-bs-dismiss="alert" id="alert-close" aria-label="Close">x</button>
    </div>
    <?php endif; ?>
    <?php endif; ?>
    <!-- Navbar -->
    ...
    <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="">
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item d-none d-sm-inline-block">
          <a href="<?php echo base_url('admin/dashboard'); ?>" class="nav-link" id="mainNav">Home</a>
        </li>
      </ul>
        ...
        if(in_array('viewAdmin', $user_permission)){
            ...
            $lastThreeNotification = $ci->Model_notification->getLastThreeNotificationForIndividual();
        } elseif(isset($_SESSION['registered_as']) && $_SESSION['registered_as'] == 1) {
            $user_type = 'organisation';
            
            $lastThreeNotification = $ci->Model_notification->getLastThreeNotificationForOrganisation();
        } else {
            $user_type = 'website';
        }
...
<?php
    if ($notificationCount){
         $nam = $notification['id'] . $notification['title'] . $notification['created_at'] . ($notification['admin'] ? 'A' : '') . ($notification['website'] ? 'W' : '') . ($notification['organisation'] ? 'O' : '') . ($notification['individual'] ? 'I' : '') . $notification['expiration_date'];
        ...
        //   }
            ...
            if ($user_type == 'admin' && $notification['admin']){
                $show = 1;
                $nam .= $user_type;
                
                // $lastThreeNotification = $ci->Model_notification->getLastThreeNotificationForAdmin();
            } elseif ($user_type == 'organisation' && $notification['organisation']) {
                $show = 1;
                $nam .= $user_type;
                
                // $lastThreeNotification = $ci->Model_notification->getLastThreeNotificationForOrganisation();
            } elseif ($user_type == 'individual' && $notification['individual']) {
                $show = 1;
                $nam .= $user_type;
...
<script>
    ...
    seeAllNotificationsLink.href = "<?= base_url('admin/announcements/show_all_popups') ?>";
  ...
  <?php if (isset($show) && isset($showPopup) && ($show && $showPopup)) { ?>
    $(document).ready(function () {
        ...
        if (!getCookie(combinedData)) {
          ...
          // Calculate the expiration date (incremented by 1 day)
