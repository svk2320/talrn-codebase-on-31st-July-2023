<link media="all" rel="stylesheet" href="<?php echo base_url() . $this->config->item('css') . 'home.css' ?>">
...
<section>
    <div class="container">
        <div class="row position-relative">
            ...
            <!-- Content-->
            <div class="col-lg-6 text-container" style="margin-top: -45px;">
                <div class="main-text text-dark fw-bold landingpage-header poppins" style="font-size: 30px; margin-top: 120px;">
                    Find & Hire iOS Developers with Ease
                </div>
                
                <div class="sub-text mb-0" style="font-size: 17px;">
                  Bring the right talent to your team effortlessly with Talrn
                </div>
                
                <div class="mt-3">
                  <p class="mb-0 footer-text" style="font-size: 15px; padding-top: 5px">
                    Hire pre-vetted remote iOS developers with strong technical &
                  </p>
                  <p class="mb-0 footer-text" style="font-size: 15px;">
                    communication skills within 48 hours. 
                  </p>
                </div>
                
                <!--Book a call block-->
                <div class="position-relative bg-white shadow-lg rounded-3 my-10 mail-block" style="width: 370px; height: 50px;">
                  <input class="form-control form-control-lg" id="Email" style="width: 250px; height: 50px; padding-left: 5%;min-height: 0px;" placeholder="Your work email" aria-label=".form-control-lg example">
                  <div class="position-absolute top-0 end-0" style="display: flex; align-items: center; justify-content: center;">
                    <button class="btn btn-primary book-btn" id="bookBtn" style="font-size: 17px; height: 50px; width: 132px;">Hire iOS Dev</button>
                  </div>
                </div>
                
                <div class="vendor-container" style="margin-top: -18px;">
                    <span>
                        <span style="font-weight: 300">Looking for remote iOS dev jobs</span>
                        <span style="font-family: Poppins"> </span>
                    </span>
                    <a href="<?php echo base_url('remote-ios-jobs'); ?>">
                       <b style="text-decoration: underline; color: #2c7dfa">Apply here</b>
                    </a>
                </div>
            </div>

            <!--Cards blocks-->
            <div class="col-lg-6 mt-5 mt-lg-0 position-lg-absolute position-sm-relative top-357 offset-lg-5" style="top: 25px; right: -53%; width: 100%;">
                    <div class="code-block" style="height: 380px; padding-top: 30px; padding-left: 25px;">
                        <!-- Loop through the card data -->
                        <?php foreach ($landing_page_profiles as $key => $card): ?>
                            
                            <div class="item">
                                <a target="_blank" href="<?= base_url('profile/' . strtolower($card['profile_url']) . '/' . strtolower($card['unique_id'] )) ?>">
                                    <div class="position-relative bg-white shadow-lg rounded-3 text-start text-primary" style="width: 210.47px; height: 322px;">
                                        <img class="position-relative top-0 start-0 rounded-top" style="width: 100%; height: 226.23px; object-fit: cover;" alt="" src="<?php echo base_url($card['userPhoto']) ?>" onerror="this.onerror=null; this.src='<?=base_url('assets/img/noimage.jpg')?>'">
                                        <div class="position-relative font-weight-bold" style="font-size: 15px; color: #000; margin: 0px 10px 0px;">
                                          <?= strlen($card['last_name'] . " " . $card['first_name'][0]) > 18 ? mb_substr($card['last_name'], 0, 10) . ' ' . mb_substr($card['first_name'], 0, 1).'...' : mb_substr($card['last_name'], 0, 10) . ' ' . mb_substr($card['first_name'], 0, 1) ?>
                                        </div>
                                        <div class="position-relative" style="margin: 0px 10px 10px; font-size: 85%; font-weight: 600;">
                                          <!-- Apply the 'word-wrap' CSS property to break long words to the next line -->
                                          <div style="word-wrap: break-word; line-height: 18px;">
                                            <?= strlen($card['primary_title']) > 50 ? substr($card['primary_title'], 0, 50).'...' : $card['primary_title'] ?>
                                          </div>
                                        </div>
                                        <div class="position-relative font-weight-bold" style="font-size: 15px; color: #60697b; margin: -10px 10px 10px; font-weight: 400; font-family: Poppins">
                                          <?php echo $card['highlight_text']; ?>
                                        </div>
                                    </div>
                                </a>
                            </div>
            ...
            <div class="col-lg-12 my-4 text-center">
                 <p class="text-justify footer-text" style="margin-top: 40px; margin-bottom: 0px;"> 
                    <span> Explore </span>
                    <span style="font-weight: 600; font-family: Poppins"><?=$profiles_count ?>+</span>
                    <span> iOS developers from </span>
                    <span style="font-weight: 600; font-family: Poppins"><?=$countries_count?>+</span>
                    <span style="font-family: Poppins"> countries, delivering </span>
                    <span style="font-weight: 600; font-family: Poppins"><?=$projects_count?>+</span>
                    <span style="font-family: Poppins"> projects. </span> <br>
                </p>
                <p class="text-justify" style="margin-top: 5px;">  
                    <span style="font-family: Poppins"> Discover </span>
                    <b style="font-family: Poppins"><?=$industries_count ?>+</b>
                    <span style="font-family: Poppins"> industry expert in </span>
                    <b style="font-family: Poppins"><?=$industries_name?></b>
                    <span style="font-family: Poppins"> & more with, </span>
                    <b style="font-family: Poppins"><?=$skills_count ?>+</b>
                    <span style="font-family: Poppins"> technology specialists  in </span>
                    <b style="font-family: Poppins"><?=$skills_name?></b>
                    <span style="font-family: Poppins"> & more </span>
                </p>
            </div>
        </div>
    </div>
    ...
    document.getElementById("bookBtn").addEventListener("click", function() {
        ...
        if (validateEmail(email)) {
            ...
            document.getElementById('bookBtn').addEventListener('click', function(event) {
              ...
              location.href = '<?php echo base_url('hire'); ?>'; // Redirect to hire page
            });
        
        } else {
          // Email is not valid, display error message if not already displayed
...
<script type="text/javascript">
  function moveItemToLastPosition(items) {
    var firstItem = items[0];

    firstItem.parentNode.removeChild(firstItem);
    items[items.length - 1].parentNode.insertBefore(
      firstItem,
      items[items.length - 1].nextSibling
    );
    firstItem.classList.remove("hide");
    firstItem.classList.remove("animation-box");
  }
  ...
  async function animateItems() {
    ...
    while (true) {
      ...
      items[0].classList.add("animation-box");

      delayValue += 2000;

      await delay(delayValue);
      items[0].classList.add("hide");
      items[1].classList.add("move-items");
      items[2].classList.add("move-items");
      items[3].classList.add("move-items");
      items[4].classList.add("move-items");

      await delay(delayValue);
      items[1].classList.remove("move-items");
      items[2].classList.remove("move-items");
      items[3].classList.remove("move-items");
      items[4].classList.remove("move-items");
        
      moveItemToLastPosition(items);

      await delay(500);
      items[0].classList.remove("move-items");
  ...
  window.addEventListener("DOMContentLoaded", animateItems);
</script>
